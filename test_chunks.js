...
// List of chunks from previous step
...
async function searchChunks() {
    for (const chunk of allChunks) {
        try {
            ...
            // Look for API endpoints related to chapters
            if (text.includes('/chapters/') || text.includes('/chapter/')) {
                console.log(`\n=== MATCH in ${chunk} ===`);
                // Find surrounding text
                const matches = text.matchAll(/"\/chapters\/[^"]+"/g);
                for (const m of matches) console.log(m[0]);

                const matches2 = text.matchAll(/"\/chapter\/[^"]+"/g);
                for (const m of matches2) console.log(m[0]);

                // Look for axios calls
                if (text.includes('.get(') || text.includes('.post(')) {
                    // extract URLs in get/post calls
                    const apiCalls = text.match(/\.(get|post)\s*\(\s*["'`][^"'`]+["'`]/g);
                    if (apiCalls) apiCalls.forEach(c => console.log(c));
                }
            }
        } catch (e) { }
    }
